...
function CreateShopModal({ isOpen, onClose, onCreated }) {
  const titleId = useId();
  const { user, isAuthenticated, linkShop } = useAuth();
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      if (!isAuthenticated || !user) {
        setError("Sign in required to create a shop.");
        return;
      }
      ...
      try {
        ...
        setError(
          err?.code === "permission-denied"
            ? "Permission denied. Deploy Phase 2 Firestore rules and ensure you are signed in."
            : err?.message || "Could not save your shop. Try again."
        );
      } finally {
        setSubmitting(false);
      }
    },
    [shopName, location, onClose, onCreated, isAuthenticated, user, linkShop]
  );
    ...
    >
      ...
      >
        ...
        <p className="mt-1.5 text-sm text-[#6b7280]">
          List your business on InquireHub. Creating a shop upgrades your account
          to vendor.
        </p>

        {!isAuthenticated ? (
          <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            <p>You need an account before creating a shop.</p>
            <Link
              to="/login"
              className="mt-3 inline-flex font-semibold text-[#0F6B36] hover:underline"
              onClick={onClose}
            >
              Sign in →
            </Link>
